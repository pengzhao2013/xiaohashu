...
/**
 * 定时分片广播任务：对当日新发布笔记的用户笔记发布数进行对齐
 *
 * @Author : zpstart
 * @Date: 2025-06-14 20:59
 */
@Component
@Slf4j
    ...
    public void notePublishCountShardingJobHandler() throws Exception {
        ...
        for (;;) {
            ...
            userIds.forEach(userId -> {
                // 2: 对 t_note 笔记表执行 count(*) 操作，获取该用户发布笔记总数
            ...
            // 4. 批量物理删除这一批次记录
            deleteMapper.batchDeleteDataAlignPublishCountTempTable(tableNameSuffix, userIds);
