...
public class TodayNoteCollectIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        // ------------------------- 笔记的收藏数变更记录 -------------------------
        // 笔记对应的 Bloom Key
        String noteBloomKey = RedisKeyConstants.buildBloomUserNoteCollectNodeIdListKey(date);
        ...
        log.info("判断NoteCollect note维度布隆过滤器,result:{}(1 表示已存在，0 表示不存在)", result);
        ...
        log.info("判断NoteCollect user维度布隆过滤器,result:{}(1 表示已存在，0 表示不存在)", result);
    }
}
