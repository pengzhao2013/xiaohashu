...
public class RelationServiceImpl implements RelationService {
    ...
    public Response<?> follow(FollowUserReqVO followUserReqVO) {
        ...
        if (Objects.equals(result, LuaResultEnum.ZSET_NOT_EXIST.getCode())) {
            log.debug("redis中没有{}对应的ZSET", followingRedisKey);
            // 从数据库查询当前用户的关注关系记录
            ...
            if (CollUtil.isEmpty(followingDOS)) {
                log.debug("数据库中也没有{}对应的关注用户数据,新增关注数据", followingRedisKey);
                ...
                log.debug("数据库中{}对应的关注用户数据不为空,新增关注数据", followingRedisKey);
                // 构建 Lua 参数
        ...
        rocketMQTemplate.asyncSendOrderly(destination, message, hashKey, new SendCallback() {
            ...
            public void onSuccess(SendResult sendResult) {
                log.info("==> 关注操作MQ 发送成功，SendResult: {}", sendResult);
            }

            @Override
            public void onException(Throwable throwable) {
                log.error("==> 关注操作MQ 发送异常: ", throwable);
            }
        });
    ...
    public Response<?> unfollow(UnfollowUserReqVO unfollowUserReqVO) {
        ...
        rocketMQTemplate.asyncSendOrderly(destination, message, hashKey, new SendCallback() {
            @Override
            public void onSuccess(SendResult sendResult) {
                log.info("==> 取关操作MQ 发送成功，SendResult: {}", sendResult);
            }

            @Override
            public void onException(Throwable throwable) {
                log.error("==> 取关操作MQ 发送异常: ", throwable);
            }
        });
    ...
    private void syncFansList2Redis(Long userId) {
        ...
        if (CollUtil.isNotEmpty(fansDOS)) {
            ...
            // 执行 Lua 脚本，批量同步关注关系数据到 Redis 中
            DefaultRedisScript<Long> script = new DefaultRedisScript<>();
            // 批量同步粉丝列表数据，可以共用 follow_batch_add_and_expire.lua 脚本
